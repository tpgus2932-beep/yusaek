...
def build_collab_router(
    *,
    ...
    @router.patch("/shopping-events/{event_id}")
    def update_shopping_event(event_id: int, payload: dict = Body(...), user: str = Depends(get_current_user)):
        region = (payload.get("region") or "").strip()
        title = (payload.get("title") or "").strip()
        content = (payload.get("content") or "").strip()
        md = (payload.get("md") or "").strip()
        ...
        cursor = conn.execute(
            ...
            SET region = ?, title = ?, content = ?, md = ?, start_date = ?, end_date = ?
            WHERE id = ?
            """,
            (region, title, content, md, start_date, end_date, event_id),
        )
        conn.commit()
        if cursor.rowcount == 0:
            conn.close()
            raise HTTPException(status_code=404, detail="해당 행사를 찾을 수 없습니다.")
        row = conn.execute("SELECT * FROM shopping_events WHERE id = ?", (event_id,)).fetchone()
        conn.close()
        return {
            "ok": True,
            "event": {
