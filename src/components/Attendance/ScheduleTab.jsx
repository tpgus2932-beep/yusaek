  ...
  const loadAttendanceRecords = useCallback(async () => {
    try {
      const params = new URLSearchParams({
        ...
        date_to: format(addDays(weekStart, 4), "yyyy-MM-dd"),
      });
      const res = await fetch(`${API}/attendance/records?${params}`);
      if (!res.ok) throw new Error("실제 출퇴근 기록을 불러오지 못했습니다.");
      setAttendanceRecords(await res.json());
    } catch (err) {
      setAttendanceRecords([]);
      setScheduleError(err.message || "실제 출퇴근 기록을 불러오지 못했습니다.");
    }
  }, [pin, weekStart]);
      ...
      <section className={styles.table}>
        ...
        {members.map((member) => (
          <div className={styles.row} key={member.id}>
            ...
            <div className={`${styles.cell} ${styles.total}`}>{formatHours(showActualWorkTimes ? actualWeeklyHours(member) : weeklyHours(member.id))}</div>
          </div>
        ))}
