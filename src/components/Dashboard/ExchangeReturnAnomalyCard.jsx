...
// 오회수 큐에 쌓이는 값(스캔 원본, 숫자만)과 이상현상의 반품송장번호
...
export default function ExchangeReturnAnomalyCard() {
    ...
    const fetchList = useCallback(async () => {
        try {
            const res = await fetch(`${API}/exchange-return-anomaly/list`, { headers: authHeaders });
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (cancelled) return;
            try {
                const res = await fetch(`${API}/exchange-return-anomaly/run`, {
                    method: 'POST',
                    headers: authHeaders,
                });
    ...
    const handleManualRefresh = async () => {
        setRefreshing(true);
        try {
            const res = await fetch(`${API}/exchange-return-anomaly/run?force=true`, {
                method: 'POST',
                headers: authHeaders,
            });
            if (handleUnauthorized(res)) return;
